...
| Transaction history + CSV export of allocations | ✅ (own only) | ✅ (all) |
...
just a name. **Two separate fields track "who this trip is for" vs "who's
actually accountable for it":** `requester` is free text on the New
Requisition form (editable — e.g. an assistant filing on someone else's
behalf), while `submitted_by` is always the signed-in session's name,
set server-side and never trusted from the client. Ownership checks
(cancelling a request, My Requests, the "mine" scope on Transaction
History) all key off `submitted_by`, not the editable `requester` field.

Every login (both roles) is recorded in the `users` table via
`record_login()` — admins get their `last_login` updated, and staff are
auto-registered there on first login, so there's a persistent record of
everyone who's used the system.
