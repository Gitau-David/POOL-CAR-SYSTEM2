...
// admins run allocation, fleet, and the audit trail. Dashboard is the one
// page both roles share, since both need the same live availability report.
const NAV = [
      ...
      <div className="flex min-h-screen">
        <aside className="w-64 shrink-0 border-r border-border bg-surface flex flex-col">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            {NAV.filter((item) => (item.roles as readonly string[]).includes(session.role)).map((item) => {
